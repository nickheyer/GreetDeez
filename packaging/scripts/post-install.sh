...
if [ ! -f /etc/greetd/config.toml ]; then
    if [ -f /etc/greetd/greetd.toml ]; then
        ...
        echo "Installed greetdeez config to /etc/greetd/config.toml"
    fi
elif ! grep -q greetdeez /etc/greetd/config.toml; then
    echo ""
    echo "  greetd config.toml exists but does not reference greetdeez."
    echo "  To use greetdeez, update [default_session] in /etc/greetd/config.toml:"
    echo ""
